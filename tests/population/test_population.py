...
@pytest.mark.parametrize(
    "instance, name",
    [
        pytest.param("anon_dace_instance", "NG96"),
        pytest.param("anon_dace_instance", "CD2133"),
        pytest.param("anon_dace_instance", "NG140", marks=pytest.mark.xfail),
        pytest.param("admin_dace_instance", "NG140"),
    ],
)
def test_population_query_database_keys(instance, name, request):
    ...
    # Check that the population_id is in the results
    assert name in results["name"]


@pytest.mark.parametrize(
    "instance, population_id",
    [
...
def test_population_get_columns(instance, population_id, request):
    ...
    expected_key = [
        ...
        "core_radius",
        "core_envelope_lum",
        "core_envelope_pressure",
        "core_envelope_temp",
        "core_envelope_density",
        "total_radius",
        "surface_lum",
        "surface_pressure",
        "surface_temp",
        ...
        "bondi_radius",
        "global_dt",
        "mean_plan_surface_density",
        "feeding_zone_width",
        "capture_radius",
        "mean_distance",
        "disk_midplane_temp",
        "disk_midplane_pressure",
        "type_migration",
        "plan_ejected_mass",
        "plan_accreted_mass",
        "isolation_mass",
        "gas_surface_density",
        "feeding_zone_inner_bound",
        "feeding_zone_outer_bound",
        "gap_with",
        "planet_dt",
        "outer_radius",
        "core_density",
        ...
        "gas_star_acc_rate",
        "migration_timescale",
        "ecc_damping_timescale",
        "inc_damping_timescale",
        "radiogenic_lumi",
        "shock_lumi",
        "surface_density",
        "surface_opac",
        ...
        "energy_factor",
        "radiation_pressure",
        "max_envelope_acc_rate",
        "bondi_envelope_acc_rate",
        "over_envelope_acc_rate",
        "outer_disque_radius",
        "intrisic_temp",
        "long_node",
        "long_peri",
...
def test_population_get_snapshots(instance, population_id, age, columns, request):
    ...
    results = instance.get_snapshots(
        population_id=population_id, years=age, columns=columns, output_format="dict"
    )
    # Result is not empty
    assert results
...
@pytest.mark.parametrize(
    "instance, population_id, system_id, planet_id",
    [
