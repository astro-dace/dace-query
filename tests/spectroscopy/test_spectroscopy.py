...
def test_spectroscopy_query_region(instance, sky_coord, expected_target: str, request):
    ...
    assert expected_target.lower() in map(
        lambda name: name.lower(), results["obj_id_catname"]
    )


# Test the authentication error by returning blank results for a non-public target
def test_spectroscopy_query_region_auth_error(anon_dace_instance):
    instance = SpectroscopyClass(dace_instance=anon_dace_instance)

    results = instance.query_region(
        sky_coord=SkyCoord("00h20m04s", "-64d52m29s", frame="icrs"),
        angle=Angle("0.045d"),
        limit=10,
        output_format="dict",
    )
...
def test_spectroscopy_download(instance, file, request):
    dace_instance: DaceClass = request.getfixturevalue(instance)
    instance = SpectroscopyClass(dace_instance=dace_instance)
    filters = {"file_rootpath": {"equal": [file]}}
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "anon_dace_instance",
            [
                "coralie14/DRS-3.8/reduced/2017-08-05/CORALIE.2017-08-06T10:12:09.000.fits"
            ],
        )
    ],
)
...
def test_spectroscopy_get_timeseries_auth_error(anon_dace_instance):
    """Test authentication error by returning blank results for a non-public target."""
    instance = SpectroscopyClass(dace_instance=anon_dace_instance)

    result = instance.get_timeseries(
        "SW0604-1658", sorted_by_instrument=False, output_format="dict"
    )
    assert not result
